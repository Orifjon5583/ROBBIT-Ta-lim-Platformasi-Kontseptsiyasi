...
function StudentsTab({ 
    ...
    handleEditClick, editingId, handleCancelEdit,
    openXPModal // <-- YANGI PROP
}) {
                    ...
                    <tbody>
                        {filteredUsers.map(u => (
                            <tr key={u.id} className={editingId === u.id ? 'editing-row' : ''}>
                                ...
                                <td>
                                    {/* XP BERISH TUGMASI (YANGI) */}
                                    <button 
                                        className="edit-btn" 
                                        style={{backgroundColor: '#9b59b6', marginRight: '5px'}} 
                                        onClick={() => openXPModal(u)}
                                        title="XP Berish"
                                    >
                                        ★
                                    </button>

                                    <button className="edit-btn" onClick={() => handleEditClick(u)}>✏️</button>
